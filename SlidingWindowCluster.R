...
### analyse clusters for all annotated genes in the genome
# input file: allgenes.txt

allgenes <- read.table("allgenes.txt",header=T,sep="\t")


# 500kb windows, 50kb increment length
cluster_table2 <- SlidingWindowCluster(allgenes, 500000, 50000)
write.table(cluster_table2, file = "SigClusters_allgenes_500kb_50kb.txt", append = FALSE, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)


### analyse clusters for outlier genes p<0.05
# input file: musCNC_final_outliers_0.05_table.txt

#outliergenes <- read.table("musCNC_final_outliers_0.05_table.txt",header=T,sep="\t")

# 500kb windows, 50kb increment length
cluster_table5 <- SlidingWindowCluster(outliergenes, 500000, 50000)
write.table(cluster_table5, file = "SigClusters_outliergenes_500kb_50kb.txt", append = FALSE, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
